...
- (void)loadMap:(NSDictionary *)mapDictionary {
  ...
  for (NSDictionary *tileset in mapDictionary[@"tilesets"]) {
    ...
    if (path) {
      ...
      //TODO I belive there is a better way to do this than two loops, too tired to remember right now
      ...
      for (NSInteger i = 0; i < tileRows; i++) {
        for (NSInteger j = 0; j < tileColumns; j++) {
          ...
          SKTexture *texture = [SKTexture textureWithRect:CGRectMake(x, y, tileWidthPercent, tileHeightPercent)
          ...
          //TODO this looks spooky, maybe make it a helper function to get the keys?
